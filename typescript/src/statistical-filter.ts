...
export class StatisticalFilter {
  ...
  private shouldPreserveStopword(word: string, contextBefore: string[], contextAfter: string[]): boolean {
  ...
  private calculatePosImportance(words: string[]): number[] {
    return words.map((word, idx) => {
      ...
      if (StatisticalFilter.STOP_WORDS.has(lower)) {
        ...
        const contextBefore = idx > 0 ? words.slice(Math.max(0, idx - 3), idx).reverse().reverse() : [];
        const contextAfter = idx + 1 < words.length ? words.slice(idx + 1, Math.min(words.length, idx + 4)) : [];
